...
// 获取图片
async function getImage(source, input) {
    if (source === 'local') {
    ...
    if (source === 'url') {
        ...
        console.log('正在下载图片...');
        const success = await backend_1.backend.downloadImage(input, tempPath);
        if (!success) {
    ...
    if (source === 'pexels') {
        ...
        const url = await backend_1.backend.searchImagePexels(input);
        if (!url) {
        ...
        const tempPath = getTempImagePath();
        console.log('正在下载图片...');
        const success = await backend_1.backend.downloadImage(url, tempPath);
        if (!success) {
            console.error('错误：图片下载失败');
            return null;
        }
...
commander_1.program
    ...
    // 处理视频
    console.log('正在处理视频...');
    const result = await backend_1.backend.processVideo(options.input, imagePath, options.output);
    if (result.success) {
...
commander_1.program
    ...
    const result = await backend_1.backend.audioToVideo(options.input, imagePath, options.output, options.width, options.height);
    if (result.success) {
...
commander_1.program
    ...
    const url = await backend_1.backend.searchImagePexels(options.query);
    if (!url) {
        console.error('未找到图片');
        process.exit(1);
    }
    ...
    if (options.output) {
        console.log('正在下载...');
        const success = await backend_1.backend.downloadImage(url, options.output);
        if (success) {
...
commander_1.program
    ...
    // 尝试获取视频信息
    const videoInfo = await backend_1.backend.getVideoInfo(options.input);
    if (videoInfo) {
        console.log('视频信息:');
    ...
    // 尝试获取音频信息
    const audioInfo = await backend_1.backend.getAudioInfo(options.input);
    if (audioInfo) {
        console.log('音频信息:');
